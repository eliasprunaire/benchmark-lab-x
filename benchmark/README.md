...
Avant l’admission, l’opérateur renouvelle la consultation publique décrite ci-dessous, avec le contexte complet annoncé par l’API et la limite de sortie configurée. Exemple pour le profil de production :

```sh
python3 -B -m benchmark.runtime forecast-prices --model openai/gpt-6-astra \
  --input-tokens 1050000 --output-tokens 16384 --cached-input-tokens 0 \
