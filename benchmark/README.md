...
L’adaptateur OpenRouter ignore seulement les clés racine supplémentaires de valeur JSON `null` dans le résultat interprété. Les cinq champs requis restent obligatoires ; toute clé supplémentaire non nulle ou erreur de schéma du paquet ou des pièces reste refusée. Après une première réponse `scope_confirmation`, une nouvelle réponse de ce type devient une clarification ; la troisième suspend la préparation avec `SCOPE_LOOP`. Une consigne ou une pièce candidate contenant une variante de « fictif » ou « inventé » est refusée avec l’incident `FORMAT_ERROR`. Ces contrôles ne valident ni le besoin ni la référence et ne modifient pas la réponse brute ou les reçus historiques.
