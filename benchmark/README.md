...
Le reçu décrit la construction effectuée ; son authenticité doit être vérifiée depuis le job ou l’opérateur identifié. L’archive utilise le format `release.json` du candidat infra. Elle inclut les deux processus, leur commande, le stockage S1 et la configuration active du catalogue `benchmark/models.toml` ; le build refuse une archive sans ces composants. Aucun tag de livraison 0.1.0 n’est créé par ces interfaces.
...
Le relais du web vers l'exécuteur est borné par un budget dérivé du budget fournisseur : un rappel OpenRouter enchaîne l'échange puis la vérification, soit deux fois 30 secondes, auxquelles s'ajoutent les 5 secondes de travail local, donc 65 secondes. Sur la socket Unix ce budget est un délai total et non une inactivité : le reste du budget monotone est réparti sur la connexion, l'envoi et chaque réception de la réponse. Côté fournisseur, le chronomètre part avant la résolution DNS et une garde coupe la socket à l'échéance, donc l'envoi, les en-têtes et le corps sont bornés même face à une réponse goutte à goutte. Le contrôle de santé garde le seul budget local de 5 secondes. Limites assumées : l'exécuteur traite les requêtes en série et le serveur web reste mono-thread, donc une requête lente retarde les suivantes et `/readyz` peut répondre 503 pendant qu'un rappel fournisseur occupe l'exécuteur ; la résolution DNS et l'établissement de la connexion fournisseur ne sont pas interruptibles, ils consomment le budget sans être majorés par lui, donc ces 65 secondes ne garantissent pas la durée totale de bout en bout. Aucun réessai n'est ajouté : un budget épuisé est un échec observé.

Les réponses d'erreur de l'exécuteur séparent le refus de la défaillance. L'enveloppe et le type de ses champs sont validés avant tout acheminement : une enveloppe fautive donne 400. Ensuite, une autorisation refusée reste 403, une révision périmée, une opération en attente ou un budget indisponible restent 409, une validation de domaine reste 400, et une défaillance interne de stockage, de programmation, d'entrée-sortie ou de contrôle de santé donne 500 avec un message fixe qui n'affirme aucun résultat : l'état de l'action est déclaré non confirmé, à vérifier dans le dossier avant tout nouvel envoi. Le code de diagnostic part dans la journalisation standard du service ; ni message d'exception, ni trace, ni requête, ni secret n'y figurent. Le web applique la même séparation : une trame ou une réponse d'exécuteur illisible est une panne de transport et répond 503, une erreur de rendu après relais répond 500, et aucune des deux n'annonce un formulaire invalide.
...
Le cookie `benchmark_session` est opaque, `HttpOnly`, `Secure`, `SameSite=Strict`, `Path=/preparation`, sans `Domain`, avec `Max-Age=2592000` (30 jours). Une réponse réussie du parcours privé renouvelle cette durée ; une erreur, un fichier statique ou une sonde de santé ne la renouvelle pas. Le cookie reste disponible après fermeture du navigateur. Le stockage ne conserve que son SHA-256 et une identité interne, jamais la clé API dans le navigateur. La clé chiffrée expire côté exécuteur après 30 jours sans vérification fournisseur réussie ; son retour tardif ne réactive pas l’accès et ne renouvelle pas le budget. Retirer la clé conserve le cookie et les dossiers. Perdre le cookie fait perdre l’accès sans effacer les dossiers ; cette durée ne constitue pas une politique de suppression des dossiers. Aucun compte ni récupération n’est ajouté. Chaque action et pièce exige sa session propriétaire ; les POST exigent aussi le jeton CSRF. Les champs d’autorité ne sont pas acceptés dans les formulaires. Les pièces sont du texte inerte, et la référence privée de jugement reste hors du paquet demandeur/candidat.
...
python3 -B -m benchmark.runtime executor --data /chemin/prive/benchmark --socket /chemin/prive/executor.sock --preparation-assistant preparation-fallback
...
Le profil historique GLM conserve ses paramètres et sa route native OpenRouter dans `provider.only=["modal/fp8","coreweave/fp8","novita/fp8"]`, avec le même `order`, `allow_fallbacks=true`, `require_parameters=true` et `data_collection="deny"`. Il n’est plus le profil documenté par défaut. Le produit envoie une seule requête HTTP, sans boucle locale ni recherche d’une autre réponse après succès ; aucun proxy d’environnement ou autre canal n’est utilisé.
...
```sh
python3 -B -m benchmark.runtime forecast-prices --model openai/gpt-6-astra \
  --input-tokens 64000 --output-tokens 16384 --cached-input-tokens 0 \
  --preparation-assistant preparation
```

`--model`, le profil sélectionné et le relevé obtenu doivent désigner le même modèle ; une divergence refuse avant l’appel d’inférence. Le champ `preparation` retourné contient `requested_configuration` et `reserve_amount`. L’opérateur les reprend dans l’autorité S2 existante avec l’`authority_id` de l’essai et le `budget_id` de l’enveloppe S1 unique en USD. La configuration contient l’identité et l’empreinte du profil, le relevé daté, les sources, la prévision, les routes, l’unique `revision` et `model_identities` dérivées de celle-ci. Une autre identité, révision, route ou capacité n’est pas admise. Aucun compte, catalogue ou synchronisation n’est créé. Le formulaire utilisateur ne choisit aucun de ces champs.

La réserve proposée est une référence indicative calculée avec les tarifs `prompt` et `completion` du modèle dans le relevé figé, sur les 64 000 tokens d’entrée nommés par les profils de production et de secours et 16 384 tokens de sortie. Le profil historique GLM conserve sa base sur le contexte complet. Le calcul réutilise `openrouter_prices.price_row` et retient le prix le plus élevé des paliers applicables à la base choisie ; la présence d’une tarification conditionnelle reste signalée. Les prix des endpoints, leurs écarts, les économies de cache et les remises ne conditionnent pas ce calcul ; l’identité et les paramètres de chaque endpoint autorisé restent vérifiés. Si le tarif modèle manque ou est invalide, `reserve_amount` proposé est `null` : l’opérateur conserve la réserve explicite de son admission existante ou en fixe une sous l’autorité budgétaire acquise. Aucun montant nul n’est déduit de cette absence. La réserve n’est ni un coût observé ni un plafond de facture. Le relevé est conservé dans l’admission, sans actualisation silencieuse.

Après une réponse exploitable publiée en clarification, aperçu ou autre état applicatif valide, `indicative_cost` conserve le sous-total indicatif calculé avec `prompt_tokens × tarif prompt + completion_tokens × tarif completion` du modèle. Les [tokens de raisonnement sont des tokens de sortie](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens) : ils ne sont pas ajoutés une seconde fois. Le calcul ne soustrait pas le cache et n’applique pas les remises ni les frais annexes. Il conserve quantités et source datée du tarif ; le prix exact du fournisseur sélectionné n’est pas requis. Tokens ou tarifs absents ou invalides donnent une estimation non estimable, sans blocage financier automatique. Une erreur 429, un timeout ou une sortie refusée ne produit pas d’estimation de succès. La vue propriétaire indique « Estimation indicative » et « ce montant n’est pas une facture », séparément du débit rapporté lorsqu’il existe. Les anciens reçus ne sont ni recalculés ni réécrits.
...
L’adaptateur OpenRouter ignore seulement les clés racine supplémentaires de valeur JSON `null` dans le résultat interprété. Les cinq champs requis restent obligatoires ; toute clé supplémentaire non nulle ou erreur de schéma du paquet ou des pièces reste refusée. Dans l’exécuteur `preparation.publish`, après une première réponse `scope_confirmation`, une nouvelle réponse de ce type devient une clarification ; la troisième suspend la préparation avec `SCOPE_LOOP`. Une consigne ou une pièce candidate contenant une variante de « fictif » ou « inventé » est refusée par ce même exécuteur avec l’incident `FORMAT_ERROR`. Ces contrôles ne valident ni le besoin ni la référence et ne modifient pas la réponse brute ou les reçus historiques.
...
Les pièces candidates restent distinctes de la référence réservée au jugement. Le contrôle de structure ne qualifie pas la vérité du contenu. Les [tests HTTP simulés](../tests/test_openrouter_preparation.py) couvrent les profils versionnés, les corrections et accords, les gardes de format, le coût inconnu, le refus avant réservation, l’absence de clé, la route rapportée et l’absence de rejeu. Ils ne prouvent ni accès du compte, ni qualité des modèles, ni fonctionnement réel sous Linux.
...
La spécification structurée contient `result_expected`, `obligations`, `eliminatory_errors`, `reference_piece_ids`, `method`, `witnesses`, `secondary_criteria`, `aggregation`, `cost_basis`, `exposure`, `professional_review`, `limits` et `local_criterion_ids`. Ce dernier champ liste explicitement les critères dont les preuves opérationnelles restent hors de la projection du juge ; une liste vide signifie qu’aucun critère n’est local. Les contrats historiques dépourvus de ce champ restent lisibles avec leur classement lexical antérieur, mais toute nouvelle version doit le déclarer. Les obligations justifient leur usage et leurs tolérances ; obligations et erreurs relient leurs identifiants de contrôle à une méthode identifiée et versionnée, sa preuve attendue et son responsable. Les témoins sont déclarés avant le contrôle. Une référence doit appartenir à la même révision et être réservée au jugement. Les autres champs supplémentaires sont refusés.
...
Le format public, sa vérification, sa lecture et son activation vivent dans [publications.py](publications.py), séparé des comparaisons privées. Ce module ne connaît ni campagne, ni évaluation, ni préparation, ni qualification : il valide des octets déjà approuvés, n’ouvre aucun stockage et n’instancie aucun `Store`. Il réutilise les validateurs purs de [validation.py](validation.py), partagés avec la préparation et la qualification. Le serveur public lit ses projections par `publications` seul ; `restitution.py` construit l’aperçu privé et importe de `publications` le schéma et la version active.
...
| `publications.materialize(bundle, approval, destination)` | Vérification des octets et du reçu fictif, puis activation atomique dans un répertoire local existant |
| `publications.public_bytes(destination, identity, name)` | Lecture d’un fichier approuvé du seul répertoire public, après vérification du manifeste, du reçu et des octets |
...
Les campagnes préparées pour le demandeur figent `CANDIDATE_SYSTEM_PROMPT` comme consigne système commune et estiment les tokens d’entrée avec `BYTES_PER_TOKEN = 3`, hypothèse conservatrice à recaler avec `prompt_tokens` du premier reçu réel.

Le plafond d’une campagne demandeur est conservé dans la table S4 dédiée `s4_caps`, séparée du manifeste gelé. Il vaut 50,00 USD par défaut, reste modifiable entre 0,10 et 100,00 USD avant l’admission, puis devient immuable dès la première admission ou tentative. Le lancement crée l’enveloppe USD du demandeur avec ce plafond, admet et réserve les cellules dans une transaction, puis arrête les émissions restantes avec `CAP_REACHED` lorsque les coûts observés connus atteignent ce plafond. Un incident de limite OpenRouter conserve son motif fournisseur et ne devient pas `CAP_REACHED`.

Les routes du parcours demandeur signalent une étape préalable absente avec `STEP_INCOMPLETE`. Le champ `step` vaut `example_validated`, `example_qualified` ou `configurations`.
...
Le bloc `provider` contient `only`, `order` (la même liste ordonnée), `allow_fallbacks`, `require_parameters: true` et `data_collection: "deny"`. Le manifeste décide les fournisseurs permis et l’autorisation de secours ; le transport ne les choisit pas. OpenRouter peut essayer les routes autorisées dans un même échange, selon sa [règle native de routage](https://openrouter.ai/docs/guides/routing/provider-selection). Aucun retry Python ou Pi n’est ajouté. Les [métadonnées de routage](https://openrouter.ai/docs/guides/features/router-metadata) sont conservées ; une route ou un effort non observé reste inconnu. Une transformation rapportée par le pipeline entraîne `HARNESS_ERROR` et conserve les octets reçus. L’éligibilité d’un modèle du catalogue constate un endpoint déclaré disponible ; elle ne présume pas la politique de données de ses hébergeurs. La conformité R-005 est imposée à chaque appel par `data_collection: "deny"` dans le bloc `provider` émis. Le `provider` observé dans le reçu, issu de l’endpoint sélectionné, est la preuve du fournisseur retenu.
...
Le transport candidat officiel réutilise Pi et la projection fermée. La commande privée `execute-candidate` conserve la sélection `--candidate-provider anthropic|deepseek|zai|openai|moonshot|dashscope|tokenhub` pour une acquisition opérateur. Une campagne peut aussi préautoriser une configuration officielle exacte : après l’épuisement prouvé des routes OpenRouter, le moteur dérive, admet, réserve et exécute ce secours sans nouvelle action sur le site. Le canal observé et toute la chaîne des reçus restent visibles dans le résultat. Les canaux sont fixes, sauf les hôtes régionaux Alibaba et Tencent fournis par `DASHSCOPE_BASE_URL` et `TENCENT_TOKENHUB_BASE_URL` : Messages Anthropic, Responses OpenAI et Alibaba Model Studio, et Chat Completions DeepSeek, Z.ai, Moonshot et Tencent TokenHub. Le transport valide les domaines régionaux officiels avant émission. Les clés ne rejoignent ni Pi ni le corps candidat. Le reçu conserve le corps natif, son empreinte, l’identité observée et les quantités natives. Sans reçu financier, le coût reste `UNKNOWN` et la réserve reste engagée.

Le manifeste de secours conserve `recovery_of` et ajoute `official_fallback: {"route_attempts": ["identifiant-operation"]}`. Ces opérations antérieures doivent prouver des incidents de route, sorties vides ou troncatures épuisées sur toutes les routes OpenRouter initialement autorisées, pour la même cellule, tâche et identité. Une réponse complète ou un refus ne permet pas ce secours. Une troncature n’est épuisée que si son plafond figé et autorisé est atteint, ou si augmenter la limite sur la même route n’a produit aucune progression. Sans cette preuve, les reprises OpenRouter restent prioritaires. L’autorité initiale fige sous `technical_recovery.official_fallbacks` la configuration native, sa preuve de disponibilité et sa réserve maximale ; une ancienne autorité dépourvue de ce bloc ne gagne aucun secours.

La configuration officielle conserve l’effort et utilise une correspondance explicite vers l’identité native. Le retrait du préfixe est admis pour Anthropic, Z.ai et OpenAI. Les correspondances particulières du panel courant sont `deepseek/deepseek-v4.1-flash` vers `deepseek-flash`, `moonshotai/kimi-k3` vers `kimi-k3`, `qwen/qwen3.8-max` vers `qwen3.8-max`, `qwen/qwen3.8-max-0902` vers `qwen3.8-max-0902` et `tencent/hy4-preview` vers `hy4-preview`. Toute autre substitution est refusée. La configuration est admise séparément avec son budget et ses paramètres natifs : `output_config.effort` pour Anthropic ; `reasoning.effort` et `max_output_tokens` pour OpenAI et Alibaba Responses ; `reasoning_effort` au premier niveau pour Moonshot ; `thinking.type=enabled` avec `reasoning_effort` pour DeepSeek, Z.ai et TokenHub. Les niveaux que le fournisseur remapperait sont refusés. Kimi K3 accepte seulement `reasoning_effort=max` sur son API officielle : une configuration OpenRouter ayant un autre effort ne peut pas basculer silencieusement vers ce canal. Tous les appels sont non streamés. Un ancien binaire ne sait pas lire ce nouveau manifeste : appliquer les mêmes précautions de retour arrière que pour les reçus v2.

La découverte authentifiée du 12 septembre 2026 confirme `claude-opus-5` chez Anthropic et `deepseek-flash` chez DeepSeek. La documentation officielle DeepSeek atteste que `deepseek-flash` désigne V4.1 Flash ; cette correspondance précise est donc déclarée dans le transport. L’ancien 0731 reste retiré. Le raccordement local testé avec des réponses HTTP simulées ne prouve ni un appel officiel réel ni son déploiement sur la VM.
...
`OPENROUTER_API_KEY` suffit pour ces cinq configurations. Un secours officiel peut être enchaîné automatiquement seulement si l’autorité initiale a déjà figé sa configuration, sa preuve de disponibilité et sa réserve, et si les reçus prouvent l’épuisement des routes OpenRouter. Renseigner une clé ne déclenche rien. Kimi K3 `low` ne possède pas d’équivalent natif exact, car l’API Moonshot exige `max` ; cette cellule s’arrête au lieu de modifier silencieusement l’effort.
