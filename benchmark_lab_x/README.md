...
Le transport candidat officiel réutilise Pi et la projection fermée. La commande privée `execute-candidate` sélectionne explicitement `--candidate-provider anthropic|deepseek|zai|openai|moonshot|dashscope|tokenhub` ; OpenRouter reste le défaut. Les canaux sont fixes, sauf l’hôte régional Alibaba fourni par `DASHSCOPE_BASE_URL` : Messages Anthropic, Responses OpenAI, et Chat Completions DeepSeek, Z.ai, Moonshot, Alibaba Model Studio et Tencent TokenHub. Le transport refuse une URL Dashscope qui n’est pas HTTPS, ne se termine pas par `/compatible-mode/v1` ou n’utilise pas un domaine régional officiel Alibaba ; il ajoute seul `/chat/completions`. Les clés ne rejoignent ni Pi ni le corps candidat. Le reçu conserve le corps natif, son empreinte, l’identité observée et les quantités natives. Sans reçu financier, le coût reste `UNKNOWN` et la réserve reste engagée.
...
`OPENROUTER_API_KEY` concerne le canal principal. `DEEPSEEK_API_KEY`, `ANTHROPIC_API_KEY`, `ZAI_API_KEY`, `OPENAI_API_KEY`, `MOONSHOT_API_KEY`, `DASHSCOPE_API_KEY` et `TENCENT_TOKENHUB_API_KEY` sont facultatives et réservées aux secours officiels. Alibaba exige aussi `DASHSCOPE_BASE_URL`, lié à la région de la clé. Les renseigner ne déclenche aucun appel ; le secours exige sa sélection et une admission distinctes.

Pour Qwen, conserver `DASHSCOPE_API_KEY` et `DASHSCOPE_BASE_URL`. Pour Hy4, conserver `TENCENT_TOKENHUB_API_KEY`. Les noms `API_HOST`, `OPENAPI_ENDPOINT`, `DASHCOPE_ENDPOINT` (orthographe incomplète) et `HUNYUAN_API_KEY` ne sont lus par aucun de ces deux transports et peuvent être retirés du fichier `.env` privé après vérification qu’aucun autre outil local ne les utilise.
...
`OPENROUTER_API_KEY` suffit pour ces cinq configurations. Les secours officiels correspondants sont raccordés, mais ne sont jamais automatiques : ils exigent les reçus prouvant l’épuisement des routes OpenRouter, une nouvelle admission et la sélection explicite du canal. Renseigner une clé ne déclenche rien.
