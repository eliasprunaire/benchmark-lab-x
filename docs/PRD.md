...
L’utilisateur décrit un travail qu’il souhaite comparer. Le produit l’aide à le préciser et construit un dossier fictif qu’il peut examiner et modifier avant de valider le besoin représenté. Une campagne autorisée compare ensuite les configurations de modèle sous le même harnais Pi. Résultats, preuves, qualités et coûts observés alimentent des classements par critère et des filtres. Le produit peut conseiller une configuration conforme qui n’est moins bonne qu’aucune autre réponse conforme sur les critères de qualité. Entre réponses de même qualité, il retient la moins coûteuse. Il ne fabrique aucun score combiné, ne désigne pas de gagnant lorsque les qualités restent incomparables et un coût faible ne rend pas une sortie erronée utilisable.
...
Les tests génériques de mathématiques ou de programmation sans besoin de travail sont hors du périmètre de Bench-X. Leur préparation s’arrête avec un motif clair et, lorsque le domaine est reconnu, des liens vers des benchmarks spécialisés sélectionnés par le produit. Ce dossier ne peut plus être poursuivi ni envoyé en comparaison. Un besoin métier vague reste à préciser ; une tâche concrète nécessitant du calcul ou du code reste admissible.
...
| Cas et résultats nécessaires au traitement | Serveur privé ; accès fermé après 7 jours d’inactivité de chaque dossier |
| Historique personnel | IndexedDB du navigateur, jusqu’à son effacement par l’utilisateur ou le navigateur |
| Contribution facultative | Copie privée distincte ; consentement échu 6 mois calendaires après son enregistrement, copie fermée à l’échéance |
| Sauvegardes | Jusqu’à 7 points quotidiens ; des copies restaurables subsistent après la fermeture de l’accès, sans durée publique tant que l’inventaire n’est pas vérifié |
...
La durée des sauvegardes reste distincte de la durée active des données. Une restauration applique les révocations et expirations avant tout accès. Aucune durée de conservation en sauvegarde n’est annoncée tant que l’inventaire n’est pas vérifié ; une telle borne ne constituerait pas une certification d’effacement physique du support.
