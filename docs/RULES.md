...
**Aide au choix économique.** Dans les résultats privés, le bloc « Notre conseil » peut nommer la configuration `SATISFAIT` au coût observé strictement le plus bas parmi les réponses conformes, sans supposer une priorité de l’utilisateur. Il explique cette base de choix. Cette aide exige un seul cas, une tentative par configuration, toutes les cellules prévues évaluées de manière concluante, au moins deux réponses conformes et leurs coûts connus et comparables. Elle est absente en cas d’égalité au minimum, de travail restant ou de couverture incomplète. Elle repose sur toute la comparaison, indépendamment des filtres. Les détails et réserves restent accessibles depuis le tableau. Elle ne promet aucune fiabilité générale, ne pondère pas la qualité et ne crée ni nouvelle mesure ni nouveau verdict. Les preuves et conclusions déjà enregistrées restent inchangées ; cette lecture des observations n’est pas une requalification.
