...
**Référence insuffisante.** Une incertitude portant sur l’attendu ou le contrôle se distingue d’une erreur candidate. Avant la comparaison, une obligation ou une erreur éliminatoire sans méthode suffisamment étayée empêche d’approuver la tâche en l’état. Après acquisition, conserver les observations et expliciter la limite : un défaut prouvé indépendamment reste `NE SATISFAIT PAS` ; sinon la finalisation attend la résolution du jugement. Corriger la référence suit les règles de versionnement du contrat, sans réécrire l’histoire. Une erreur de juge se traite sur la sortie conservée ; elle ne déclenche pas un nouvel appel candidat. Une erreur de recopie d’empreinte, avec pièce autorisée et passage exact, est une anomalie technique corrigible par le serveur, pas un échec candidat ni un motif de masquage du résultat. La correction est liée au reçu inchangé et à la version du traitement ; elle ne modifie ni citation ni constat métier. Une pièce étrangère ou un passage absent reste une preuve inexploitable. Une reprise technique doit traiter un incident identifié et conserver chaque tentative, jamais sélectionner des essais jusqu’à obtenir un succès.
...
6. classements par critère et filtres de consultation, avec l’aide économique conditionnelle définie ci-dessous, sans choix automatique ni classement global de qualité.
...
**Complétude économique.** La conclusion économique décrit le périmètre de comparaison des coûts, les valeurs connues, les inconnues et les incompatibilités. Elle porte `INCOMPLETE` lorsque les données de ce périmètre ne permettent pas une comparaison complète. Les coûts connus restent visibles et un coût inconnu ne retire pas l’admissibilité sur les critères non économiques. `INCOMPLETE` est un état économique distinct du verdict. Si le coût est une obligation figée avant exécution, un coût `INCONNU` interdit de la déclarer satisfaite.

**Aide au choix observé.** Dans les résultats privés, le bloc « Notre conseil » peut nommer une configuration `SATISFAIT` qui n’est moins bonne qu’aucune autre réponse conforme sur chaque critère secondaire ordonné. Si plusieurs réponses partagent exactement cette meilleure qualité, il retient celle dont le coût observé est strictement le plus bas. Il explique toujours cette base de choix. Cette aide exige un seul cas, une tentative par configuration, toutes les cellules prévues évaluées de manière concluante, au moins deux réponses conformes, des critères secondaires connus et comparables, ainsi que des coûts connus. Elle est absente lorsque plusieurs compromis de qualité restent non dominés, en cas d’égalité finale, de travail restant ou de couverture incomplète. Elle repose sur toute la comparaison, indépendamment des filtres. Elle ne promet aucune fiabilité générale, ne pondère pas les critères et ne crée ni score, ni mesure, ni verdict. Les preuves et conclusions déjà enregistrées restent inchangées ; cette lecture des observations n’est pas une requalification.

**Préparation et jugement.** Interview, génération du dossier, correction et jugement assisté exigent une autorité d’appel et une enveloppe identifiées avant consommation, éventuellement accordées par l’opérateur autorisé dans son périmètre. Les critères dont les preuves opérationnelles restent locales sont identifiés explicitement dans le contrat ; aucun mot d’un cas métier ne suffit à les reclasser. Leurs dépenses sont relevées séparément des appels candidats. Le temps humain n’est pas monétisé sans méthode décidée et mesure correspondante. Leur inclusion éventuelle dans une base de coût est déclarée avant comparaison, avec une imputation commune ; elles ne sont ni dissimulées dans le coût candidat ni réputées nulles. L’écran de lancement d’une campagne n’autorise pas rétroactivement les dépenses de préparation.

**Contrôle de dépense.** Dans le parcours financé par le demandeur, le plafond non renouvelable de sa clé OpenRouter est la seule limite financière. Aucun plafond local quotidien, par navigateur ou par campagne ne s’y ajoute. L’accès doit rester valide et disposer de crédit ; les appels demeurent soumis à leur autorisation. Prévision, réservation avant appel et coût observé restent distincts et conservés. Les anciennes enveloppes locales ne bloquent pas ce parcours et ne représentent pas le crédit disponible chez le fournisseur. Les campagnes opérateur conservent les contrôles de leur enveloppe explicite. Un coût manquant reste inconnu, sans libérer sa réservation ni fabriquer un solde. Un contrôle local ne prouve pas un plafond absolu de facturation. Un coût local n’est pas nul par défaut.
...
**Aucun visuel trompeur.** Aucun graphique ou classement n’implique une échelle ou une précision absentes du contrat. Aucun score combiné n’est produit tant que sa méthode n’est pas décidée. L’interdiction de podium général ou de classement universel reste durable.
...
**Pas d'anticipation.** Réutiliser les primitives retenues dans l’ARD. Aucun microservice, Kubernetes, bus de messages, système de plugins, moteur de score pondéré ou abstraction spéculative n’est ajouté sans besoin démontré. Le parcours public, les tris et les filtres sont décidés ; ils n’autorisent pas une gestion de comptes, une politique de publication ou une formule de score encore non choisies.
...
**Jalons.** Un jalon porte un nom, jamais un numéro de version : alpha, bêta, puis release candidate 1, 2, 3. Il ne se confond pas avec un suffixe de préversion SemVer comme `alpha.N`. La [feuille de route](roadmap.md) relie chaque exigence du PRD à son jalon ; le numéro de chaque version reste calculé à la release. Un jalon ne requalifie aucun résultat déjà scellé et ne prouve aucune livraison.
