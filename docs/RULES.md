...
**Aide au choix économique.** Dans les résultats privés, si le coût est la priorité de l’utilisateur, la vue peut nommer la configuration `SATISFAIT` au coût observé strictement le plus bas parmi les réponses conformes. Cette aide exige un seul cas, une tentative par configuration, toutes les cellules prévues évaluées de manière concluante, au moins deux réponses conformes et leurs coûts connus et comparables. Elle est absente en cas d’égalité au minimum, de travail restant ou de couverture incomplète. Elle repose sur toute la comparaison, indépendamment des filtres, et renvoie aux détails et réserves. Elle ne promet aucune fiabilité générale, ne pondère pas la qualité et ne crée ni nouvelle mesure ni nouveau verdict. Les preuves et conclusions historiques restent inchangées ; cette lecture des observations n’est pas une requalification.
