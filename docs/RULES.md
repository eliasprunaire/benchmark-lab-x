...
**Aide au choix observé.** Dans les résultats privés, le bloc « Notre conseil » peut nommer une configuration `SATISFAIT` qui n’est moins bonne qu’aucune autre réponse conforme sur chaque critère secondaire ordonné et sur le coût du cas, et qui est strictement meilleure sur au moins un de ces axes. Lorsque leurs qualités observées sont exactement à égalité, il peut retenir celle dont le coût observé est strictement le plus bas. Il explique toujours cette base de choix. Cette aide exige un seul cas, une tentative par configuration, toutes les cellules prévues évaluées de manière concluante, au moins deux réponses conformes, des critères secondaires connus et comparables, ainsi que des coûts connus. Elle est absente lorsque plusieurs compromis restent non dominés, en cas d’égalité finale, de travail restant ou de couverture incomplète. Elle repose sur toute la comparaison, indépendamment des filtres. Elle ne promet aucune fiabilité générale, ne pondère pas les critères et ne crée ni score, ni mesure, ni verdict. Les preuves et conclusions déjà enregistrées restent inchangées ; cette lecture des observations n’est pas une requalification.
