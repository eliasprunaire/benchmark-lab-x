...
Tests proches : `test_official_transport`, `test_recovery`, `test_s14_judgment`, `test_s14_acceptance`, `test_private_comparison`, `test_s5_regressions`, `test_s6_regressions`. Validation de livraison : `uv run --with requests --with mpmath==1.3.0 python -m unittest discover -s tests`, puis `benchmark/test_demo.py` séparément. Les nouvelles régressions portent sur les défauts observés et les chemins opérateur.
