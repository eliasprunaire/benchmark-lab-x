# Fil de courriels — devis de prestation (présenté hors ordre chronologique)

Les messages ci-dessous sont présentés **hors ordre chronologique**. Les champs Expéditeur, Destinataire, Date et Objet sont complets. Toutes les entités sont synthétiques.

---

### Message A

**Expéditeur :** claire.renaud@test-alpha.example
**Destinataire :** m.perrin@studio-test.example
**Date :** 12 février 2026, 9 h 14
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Bonjour Matthieu,

J'ai bien reçu le devis V2. Nous approuvons le montant de **4 850 € HT** et le calendrier que vous avez indiqué (démarrage pendant la semaine du 2 mars).

Pouvez-vous me confirmer que la livraison des maquettes pour ordinateur et mobile est toujours prévue le **13 mars 2026** ?

Bien cordialement,
Élodie Martin
Gérante — Test Alpha SAS

---

### Message B

**Expéditeur :** m.perrin@studio-test.example
**Destinataire :** claire.renaud@test-alpha.example
**Date :** 5 février 2026, 11 h 02
**Objet :** Devis de refonte du site vitrine — Test Alpha

Bonjour Claire,

À la suite de notre échange téléphonique de lundi, vous trouverez ci-joint le devis no TA-2026-001 pour la refonte du site vitrine d'Test Alpha.

**Montant :** 4 200 € HT
**Périmètre :** identité visuelle épurée, 5 pages, intégration WordPress, formation de 2 heures
**Devis valable jusqu'au :** 28 février 2026
**Livraison des maquettes pour ordinateur et mobile :** 13 mars 2026
**Mise en ligne visée :** 3 avril 2026

N'hésitez pas à me contacter si vous avez des questions.

Bien cordialement,
Alex Durand
Studio Test SAS — 12 rue des Tilleuls, 69002 Lyon

---

### Message C

**Expéditeur :** claire.renaud@test-alpha.example
**Destinataire :** m.perrin@studio-test.example
**Date :** 18 février 2026, 16 h 40
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Matthieu,

Pouvez-vous m'envoyer une version PDF à signer du devis V2, avec le montant et les dates arrêtés comme convenu ?

Nous devons la transmettre à notre comptable avant le 21 février.

Merci,
Claire

---

### Message D

**Expéditeur :** m.perrin@studio-test.example
**Destinataire :** claire.renaud@test-alpha.example
**Date :** 10 février 2026, 14 h 27
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Claire,

J'ai bien noté l'ajout des pages « Réalisations » et « Matériaux ». Cela augmente le volume de contenu et le temps d'intégration.

Je vous enverrai un devis révisé (**V2**) sous 48 heures.
Nouveau montant proposé : **4 850 € HT** (au lieu de 4 200 € HT).
Pour le moment, je maintiens la livraison des maquettes au **13 mars 2026**, sauf retard du cahier des charges photographique.

Bien à vous,
Alex Durand

---

### Message E

**Expéditeur :** claire.renaud@test-alpha.example
**Destinataire :** m.perrin@studio-test.example
**Date :** 6 février 2026, 8 h 55
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Bonjour Matthieu,

Merci pour le devis. Le montant de 4 200 € HT respecte notre budget.

Nous aurons toutefois besoin d'ajouter deux pages : « Réalisations » et « Matériaux ». Est-ce possible sans déplacer la mise en ligne du 3 avril ?

Par ailleurs, qui assurera l'hébergement après la mise en ligne : vous, ou devons-nous nous en charger ?

Bien cordialement,
Élodie Martin

---

### Message F

**Expéditeur :** m.perrin@studio-test.example
**Destinataire :** claire.renaud@test-alpha.example
**Date :** 11 février 2026, 10 h 05
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Bonjour Claire,

Voici le devis V2 (no TA-2026-001-R) :

- **Montant :** 4 850 € HT
- **Périmètre :** 7 pages (les 5 pages initiales + Réalisations + Matériaux), identité visuelle épurée, WordPress, formation de 2 heures
- **Livraison des maquettes :** 13 mars 2026
- **Mise en ligne visée :** 3 avril 2026
- **Valable jusqu'au :** 28 février 2026

Dès réception de votre accord écrit, nous réserverons le créneau de démarrage pendant la semaine du 2 mars.

Bien cordialement,
Alex Durand

---

### Message G

**Expéditeur :** m.perrin@studio-test.example
**Destinataire :** claire.renaud@test-alpha.example
**Date :** 13 février 2026, 15 h 18
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Claire,

Oui : les maquettes pour ordinateur et mobile seront livrées le **20 mars 2026** (et non le 13). De votre côté, le cahier des charges photographique de vos projets n'est pas encore terminé ; je préfère prévoir une semaine supplémentaire plutôt que de promettre le 13.

Le montant reste de **4 850 € HT**. La mise en ligne reste visée au **3 avril 2026**.

Dites-moi si cela vous convient.

Matthieu

---

### Message H

**Expéditeur :** claire.renaud@test-alpha.example
**Destinataire :** m.perrin@studio-test.example
**Date :** 9 février 2026, 17 h 33
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Matthieu,

Oui pour les deux pages supplémentaires. Nous pouvons fournir les textes avant le 27 février.

Nous attendons toujours votre réponse concernant l'hébergement : l'offre l'inclut-elle pour la première année, ou couvre-t-elle uniquement la livraison du site ?

Bien cordialement,
Claire

---

### Message I

**Expéditeur :** m.perrin@studio-test.example
**Destinataire :** claire.renaud@test-alpha.example
**Date :** 19 février 2026, 9 h 02
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Bonjour Claire,

Vous trouverez ci-joint le PDF à signer du devis V2 (no TA-2026-001-R).

Récapitulatif arrêté :
- Montant : **4 850 € HT**
- Livraison des maquettes : **20 mars 2026**
- Mise en ligne visée : **3 avril 2026**
- Acompte à la commande : 40 %

Dès réception de l'exemplaire signé, nous confirmerons le démarrage pendant la semaine du 2 mars.

Bien cordialement,
Alex Durand
Studio Test SAS

---

### Message J

**Expéditeur :** claire.renaud@test-alpha.example
**Destinataire :** m.perrin@studio-test.example
**Copie :** comptabilite@test-alpha.example
**Date :** 20 février 2026, 11 h 47
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Matthieu,

Devis signé en pièce jointe. L'acompte de 40 % sera viré d'ici au samedi 21 février.

Pouvez-vous me confirmer par retour que le montant de **4 200 € HT** figurant sur notre bon de commande interne, repris de votre premier devis, est remplacé par **4 850 € HT** ?

Merci,
Élodie Martin

---

### Message K

**Expéditeur :** m.perrin@studio-test.example
**Destinataire :** claire.renaud@test-alpha.example
**Date :** 20 février 2026, 14 h 10
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

Claire,

Confirmation : le montant contractuel est de **4 850 € HT** (devis V2). Le montant de 4 200 € HT correspond au devis initial sans les pages supplémentaires et ne doit plus être utilisé.

Livraison des maquettes : 20 mars 2026.
Mise en ligne visée : 3 avril 2026.

J'ai bien noté que l'acompte est prévu pour demain.

Matthieu

---

### Message L

**Expéditeur :** claire.renaud@test-alpha.example
**Destinataire :** m.perrin@studio-test.example
**Date :** 14 février 2026, 10 h 22
**Objet :** Re: Devis de refonte du site vitrine — Test Alpha

La date du 20 mars pour les maquettes nous convient. Nous l'approuvons.

Un dernier point avant la signature : le devis V2 comprend-il l'hébergement et le nom de domaine pour la première année, ou devons-nous prévoir un budget séparé ? J'ai besoin de cette information pour finaliser le budget 2026.

Claire
